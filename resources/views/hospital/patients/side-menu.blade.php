<ul class="nav nav-tabs inner-tabs" id="tab-{{ $tab }}">
      ...
      <li class="{{ ( $active_tab == 'submissions')? 'active' : ''}}" onclick="window.document.location='{{ url($hospital['url_slug'].'/patients/'.$patient['id'].'/submissions') }}';"><a href="#"><i class="fa fa-list-alt"></i> Submissions</a></li>
      <li class="{{ ( $active_tab == 'base_line')? 'active' : ''}}" onclick="window.document.location='{{ url($hospital['url_slug'].'/patients/'.$patient['id'].'/base-line-score') }}';"><a href="#"><i class="fa fa-bar-chart"></i> Baseline Score</a></li>
